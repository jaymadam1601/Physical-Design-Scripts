# Smart Net Stretching Script with Adaptive Stretch Compensation

# Procedure to stretch Y-direction
...
# Calculate length to achieve target area
...
# Calculate actual possible stretch limits
proc get_stretch_limits {box_increased box_original} {
    ...
    return [list $up_len $down_len $can_stretch_up $can_stretch_down]
}

set summary {}
set skipped_fp [open "not_stretched_markers.tcl" w]
set count 0
foreach mark [dbget top.markers.userType M5.R.14 -p ] {
    ...
    set mark_box_increased [dbShape $mark_box SIZEY $half_increase]
    ...
    if {[llength [dbget selected.objType -u -e]] == 0} {
        ...
        puts $skipped_fp "createMarker -bbox $mark_box -subtype \"Not streched\" -desc \"No Nets present\""
        continue
    }
    ...
    set wire_increase_up [lindex [dbShape $mark_box_increased XOR $wire_box_before] 1]
    set wire_increase_down [lindex [dbShape $mark_box_increased XOR $wire_box_before] 0]
    set up_size_length [get_box_length $wire_increase_up]
    set down_size_length [get_box_length $wire_increase_down]

    lassign [get_stretch_limits $mark_box_increased $mark_box] max_up max_down can_up can_down
    ...
    } elseif {$can_up || $can_down} {
        set target_total [expr {$up_size_length + $down_size_length}]
        set best_up 0
        set best_down 0
        set step 0.05

        if {$can_up} {
            for {set i $up_size_length} {$i >= 0} {set i [expr {$i - $step}]} {
                set upper_box [lindex [dbShape [dbShape $wire_box_before SIZEY $i] XOR $wire_box_before] 1]
                set up_overlap [dbQuery -areas $upper_box -layers {M5} -objType {wire pWire sWire}]
                if {[llength $up_overlap] == 0} {
                    set best_up $i
                    break
                }
            }
            set best_down [expr {$target_total - $best_up}]
            if {$best_down > $max_down} { set best_down $max_down }
        } elseif {$can_down} {
            for {set i $down_size_length} {$i >= 0} {set i [expr {$i - $step}]} {
                set lower_box [lindex [dbShape [dbShape $wire_box_before SIZEY $i] XOR $wire_box_before] 0]
                set down_overlap [dbQuery -areas $lower_box -layers {M5} -objType {wire pWire sWire}]
                if {[llength $down_overlap] == 0} {
                    set best_down $i
                    break
                }
            }
            set best_up [expr {$target_total - $best_down}]
            if {$best_up > $max_up} { set best_up $max_up }
        }
        editStrechY $best_up $best_down
        append summary "\nMARK: $mark_box --> Stretching [dbget selected.net.name] Up: $best_up Down: $best_down"
    } else {
