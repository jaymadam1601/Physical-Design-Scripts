proc extract_sp_ep_from_gz {gz_file} {
	set out_file "sp_ep_slack.rpt"
	...
	set cmd "sort -u -k  1,1 $out_file | sort -nk 3 > sp_ep_slack_sort.rpt"
	set fp2 [open "sp_ep_slack_sort.rpt" w]
	puts -nonewline $fp2 [exec sh -c $cmd]
	close $fp2
	
}

proc timing_slack_check_sp_ep {file_name {num_path "100"} {result_file "setup_hold_margin_check.rpt"}} {
	extract_sp_ep_from_gz $file_name
	set fp [open "sp_ep_slack_sort.rpt" r]
	set fp1 [open $result_file w]
	set count 0
	puts $fp1 "Startpoint StartpointClockPin Endpoint EndpointClockPin N-1SetupSlack N-1HoldSlack SetupSlack HoldSlack N+1SetupSlack N+1HoldSlack"
	while {[gets $fp line] >= 0} {
		...
		set setup_path [get_timing_paths -from $start_point -to $end_point -attributes {slack startpoint endpoint_clock_pin}]
		if {[sizeof_collection $setup_path] > 0} {
			set setup_slack [get_attribute $setup_path slack]
			set setup_startpoint_clock [get_object_name [get_attribute $setup_path startpoint]]
			set setup_endpoint_clock [get_object_name [get_attribute $setup_path endpoint_clock_pin]]
		} else {
			set setup_slack NA
			set setup_startpoint_clock NA
			set setup_endpoint_clock NA
		}
		...
		puts $fp1 "$start_point $setup_startpoint_clock $end_point $setup_endpoint_clock $Nminu1setup_slack $Nminu1hold_slack $setup_slack $hold_slack $Nplus1setup_slack $Nplus1hold_slack"
